...
func TestInternalPluginManagerOnly(t *testing.T) {
	gin.SetMode(gin.TestMode)

	const token = "pm-s3cr3t-per-instance-token"
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			if tc.envToken == "" {
				t.Setenv("PLUGIN_MANAGER_INTERNAL_TOKEN", "")
			} else {
				t.Setenv("PLUGIN_MANAGER_INTERNAL_TOKEN", tc.envToken)
			}

			nextCalled := false
			r := gin.New()
			r.GET("/internal/plugin-manager/instance-stats", InternalPluginManagerOnly(), func(c *gin.Context) {
				nextCalled = true
				c.JSON(http.StatusOK, gin.H{"status": "ok"})
			})

			req := httptest.NewRequest(http.MethodGet, "/internal/plugin-manager/instance-stats", nil)
			if tc.headerTok != "" {
