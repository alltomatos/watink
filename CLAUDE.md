...
| Base de Conhecimento (RAG) — microsserviço `watink-knowledge` (Python/FastAPI) + nó `knowledge` + guardrails+citação | ✅ Concluída (PR #256) — **superada pelo RAG nativo abaixo** |
| Base de Conhecimento — CI Python (ruff+pytest) + higiene `__pycache__` | ✅ Concluída (PR #257/#258) — removida junto com o descomissionamento do microsserviço |
...
| Base de Conhecimento — **RAG nativo em Go** (ADR 0028, supera ADR 0018): schema+embeddings+retriever, scraping/parsers/chunking nativos, worker de ingestão+reconciler, Agent Runtime em Go, switch `KNOWLEDGE_MODE`, RequirePermission+cap de upload | 🔧 Implementado em PRs #482-#487, aguardando review/validação em staging antes do merge para `develop` |
...
**Arquitetura:**
- Um único processo: `business` (Go) faz CRUD de bases/fontes + UI, upload/download S3, publica **e consome** os jobs de ingestão (AMQP, `internal/knowledge/ingest_worker.go`), responde retrieval/agent in-process (`flow.Retriever`/`flow.AgentResponder`), roda um reconciler cron para fontes presas.
- `KNOWLEDGE_MODE=native|http` (default `http` durante a migração) troca a implementação por trás das duas interfaces acima sem tocar em nenhum consumidor — rollback trivial via env, sem redeploy. Ver `internal/knowledge/wiring.go`.
